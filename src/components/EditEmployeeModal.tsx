...
export function EditEmployeeModal({ isOpen, onClose, employee }: EditEmployeeModalProps) {
  const { updateEmployee } = useAppData();
  const [formData, setFormData] = useState({
    name: '', role: '', email: '', phone: '', specialties: '' as string, rating: 0, appointmentsToday: 0, avatar: '', status: 'active' as 'active' | 'inactive'
  });
  ...
  useEffect(() => {
    if (employee) {
      setFormData({
        ...
        appointmentsToday: employee.appointmentsToday,
              ...
              <div className="bg-gradient-to-r from-teal-500 to-marine-500 text-white p-5 sm:p-6">
                <div className="flex items-start justify-between gap-4">
                  <div>
                    ...
                    <p className="text-white/80 text-sm">Actualiza los datos del empleado</p>
                  </div>
              ...
              <form onSubmit={handleSubmit} className="p-5 sm:p-6 space-y-4 max-h-[calc(90vh-200px)] overflow-y-auto">
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  ...
                  <div className="space-y-2">
                    ...
                    <Input id="role" name="role" value={formData.role} onChange={handleChange} className="h-11 rounded-xl bg-slate-50 dark:bg-slate-700" />
                  </div>
                  <div className="space-y-2">
                    <Label htmlFor="email">Email</Label>
                    <div className="relative">
                      <Mail className="absolute left-3 top-1/2 -translate-y-1/2 w-5 h-5 text-slate-400" />
                      <Input id="email" name="email" type="email" value={formData.email} onChange={handleChange} className="pl-10 h-11 rounded-xl bg-slate-50 dark:bg-slate-700" />
                    </div>
                  </div>
                  <div className="space-y-2">
                    ...
                    <div className="relative">
                      ...
                      <Input id="phone" name="phone" value={formData.phone} onChange={handleChange} className="pl-10 h-11 rounded-xl bg-slate-50 dark:bg-slate-700" />
                    </div>
                  </div>
                  <div className="space-y-2 sm:col-span-2">
                    <Label htmlFor="specialties">Especialidades</Label>
                    <Input id="specialties" name="specialties" value={formData.specialties} onChange={handleChange} className="h-11 rounded-xl bg-slate-50 dark:bg-slate-700" />
                  </div>
                  <div className="space-y-2">
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="relative">
                      ...
                      <select id="status" name="status" value={formData.status} onChange={handleChange} className="w-full h-11 pl-10 pr-3 rounded-xl border border-slate-200 dark:border-slate-600 bg-slate-50 dark:bg-slate-700 text-slate-900 dark:text-white">
