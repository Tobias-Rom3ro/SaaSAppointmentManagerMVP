  ...
  return (
    <div className="p-8 space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-slate-900 mb-2">Notificaciones</h1>
        ...
        {unreadCount > 0 && (
          <motion.div whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}>
            <Button
              ...
              variant="outline"
              className="px-6 h-11 rounded-xl border-slate-200"
            >
      ...
      {/* Filter Tabs */}
      <div className="flex items-center gap-2 bg-white p-1 rounded-xl macos-shadow border-0 inline-flex">
        <button
      ...
      <div className="space-y-3">
        {filteredNotifications.length === 0 ? (
          <Card className="p-12 macos-shadow border-0 bg-white text-center">
            <div className="w-16 h-16 rounded-2xl bg-slate-100 flex items-center justify-center mx-auto mb-4">
              <Bell className="w-8 h-8 text-slate-400" />
            </div>
            <h3 className="text-slate-900 mb-2">No hay notificaciones</h3>
            <p className="text-slate-600">Cuando recibas notificaciones, aparecerán aquí</p>
          </Card>
        ) : (
          filteredNotifications.map((notification, index) => {
            ...
            return (
              ...
              >
                <Card
                  className={`p-5 macos-shadow hover:shadow-lg transition-all duration-300 border-0 cursor-pointer group ${
                    notification.read ? 'bg-white' : 'bg-gradient-to-r from-teal-50 to-marine-50'
                  }`}
                  onClick={() => markAsRead(notification.id)}
                >
                  <div className="flex items-start gap-4">
                    {/* Icon */}
                    <div className={`w-12 h-12 rounded-xl ${config.iconBg} flex items-center justify-center flex-shrink-0 macos-shadow`}>
